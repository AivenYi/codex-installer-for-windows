...
| 🔍 **スマート環境検出** | Python ≥3.11 / Node.js ≥20.x を検出し、自动インストール/アップグレード |
| 📄 **検証済み設定テンプレート** | 実績のある完全なテンプレートを直接書き込み |
| 🔄 **リレープロキシ** | codex-relay を自动インストール・設定 |
| 🚀 **自動起動設定** | システム起動時にリレーが自动起動 |
| 🛡️ **管理者権限昇格** | リレーを管理者権限で自动起動 |
| 🔒 **匿名化共有** | API Key を伏せた状态下で設定を安全に共有 |
...
> ⚠️ **注意**: 実際のインストール場所に合わせてパスを调整してください。

### 必要なもの

1. ✅ インターネットに接続された Windows PC
2. ✅ **DeepSeek API Key**（登録方法は下列参照）
3. ✅ 约 **10〜15分**（ネットワーク速度によります）
...
> 💡 **ヒント**: 新規ユ��ザーには500万トークンが無料で付与されます。
...
┌─────────────────────────────────────────────────┐
│  ④ 設定ファイル生成                        │
│     ├── Start_Deepseek_relay.ps1 ← 起動スクリプト│
│     └── config.toml          ← 設定ファイル │
└─────────────────────────────────────────────────┘
                        │
                        ▼
...
- **≥ 20.x** → スキップ
- **< 20.x** または未安装 → 自動インストール

#### ステップ ③: API Key の取得

スクリプトが DeepSeek の注册ページを開きます：
1. DeepSeek に登録/ログイン
2. API Keys → 新しいキーを作成
3. `sk-...` の文字列をコピー
...
| 問題 | 解決策 |
|------|--------|
| winget インストール失敗 | nodejs.org からのダウンロードに自动フォールバック |
| ダウンロードが遅い | https://nodejs.org/ から手動ダウンロード |
| インストール後も `node` が使えない | PowerShell を再起動 |
...
現在は DeepSeek のみ対応しています.otherモデル（OpenAI、Claude 等）を使用するには、`config.toml` を手動で編集してください。
